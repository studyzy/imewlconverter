注意：该小工具基于C#开发，所以电脑上必须安装.Net Framework 4.0才能正常运行，如果双击“深蓝词库转换.exe”后弹出错误窗口，请下载安装.Net Framework 4.0再试。
...
2.1版升级为.net 4.0，提供对Mac和Linux上Mono命令行调用。修复了之前自定义导出的Bug，增加了对各种输入法的用户自定义短语的导入导出支持，增加了批量转换时导入导出文件一对一的支持，而不是合并词库成一个文件，增加了词库合并功能。修正了文件分割的Bug和词库转换时的一些Bug。
