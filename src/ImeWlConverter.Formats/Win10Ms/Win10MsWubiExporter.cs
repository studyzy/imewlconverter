...
/// <summary>Win10 Microsoft Wubi user dictionary exporter (binary format).</summary>
[FormatPlugin("win10mswb", "Win10微软五笔（用户自定义短语）", 131, FileExtension = ".dat")]
public sealed partial class Win10MsWubiExporter : IFormatExporter
{
    public Task<ExportResult> ExportAsync(
        IReadOnlyList<WordEntry> entries, Stream output,
        ExportOptions? options = null, CancellationToken ct = default)
    {
        // Filter: code length <= 32 chars, word length <= 64 chars
        var filtered = entries
            .Where(e => e.Word.Length <= 64)
            .Where(e =>
            {
                var codeLen = e.Code?.GetPrimaryCode("").Length ?? 0;
                return codeLen <= 32;
            })
            .ToList();
        ...
        for (var i = 0; i < filtered.Count; i++)
        {
            ...
            var codeStr = entry.Code?.GetPrimaryCode("") ?? "";
            offset += 8 + 8 + codeStr.Length * 2 + 2 + entry.Word.Length * 2 + 2;
        }
    ...
    {
        var codeStr = entry.Code?.GetPrimaryCode("") ?? "";
        var hanziOffset = 8 + 8 + codeStr.Length * 2 + 2;
        ...
        // Wubi code in Unicode
        bw.Write(Encoding.Unicode.GetBytes(codeStr));
        // 00 00 separator
        bw.Write((short)0);
        // Word in Unicode
