...
public class Cangjie5Generater : BaseCodeGenerater, IWordCodeGenerater
{
            ...
            if (dictionary == null)
            {
                ...
                )
                {
                    ...
                    try
                    {
                        ...
                        var cj = new Cangjie();
                        cj.Code = arr[1];
                        if (arr.Length == 3) cj.SplitCode = arr[2];
    ...
    public override Code GetCodeOfString(string str)
    {
        ...
        if (str.Length == 1)
        {
            ...
            foreach (var cangjy in Dictionary[str[0]]) c.Add(cangjy.Code);
    ...
    public IList<string> GetAllCodesOfChar(char str)
    {
        ...
        foreach (var cangjie in Dictionary[str]) result.Add(cangjie.Code);
        return result;
    }
    ...
    {
        if (OneCodeChar.ContainsKey(c)) return new List<string> { OneCodeChar[c] };
        var x = Dictionary[c];
        var result = new List<string>();
    ...
    private IList<string> GetFirstAndLastCode(char c)
    {
        ...
        foreach (var cangjy in x)
        {
            ...
            if (cangjy.SplitCode != null && !IgnoreContainRule)
            {
                ...
                if (arr[0].Length > 1) lastCode = GetSplitedCode(cangjy.SplitCode);
            }
    ...
    private IList<string> GetFirstSecondLastCode(char c)
    {
        ...
        foreach (var cangjy in x)
            try
            {
                ...
                var code = cangjy.Code[0].ToString() + cangjy.Code[1];
                if (cangjy.Code.Length > 2)
                {
                    ...
                    if (cangjy.SplitCode != null && !IgnoreContainRule)
                    {
                        ...
                        if (arr[0].Length > 2) lastCode = arr[0][arr[0].Length - 1];
                    }
