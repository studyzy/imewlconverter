...
public class TerraPinyinGenerater : PinyinGenerater
{
    ...
    public override Code GetCodeOfString(string str)
    {
        ...
        {
            var prow = py[i];
            foreach (var p in prow) result.Add(PinyinHelper.AddToneToPinyin(str[i], p));
        }
    ...
    {
        if (wl.CodeType == CodeType.TerraPinyin) return;
        if (wl.CodeType == CodeType.Pinyin) //如果本来就是拼音输入法导入的，那么就用其拼音，不过得加上音调
        {
            for (var i = 0; i < wl.Codes.Count; i++)
            {
                ...
                {
                    var s = row[j];
                    var py = PinyinHelper.AddToneToPinyin(wl.Word[i], s); //add tone
                    wl.Codes[i][j] = py;
                }
            }
